# Gemini CLI Context: Fuse

Use this file as Gemini CLI context, for example by placing it in a project-level `GEMINI.md`, a global `~/.gemini/GEMINI.md`, or another Gemini context file imported from `GEMINI.md`.

Gemini CLI does not use the same `SKILL.md` skill package format as Claude Code or Codex. For a `/fuse` slash command, use the accompanying `fuse.toml` file. This Markdown file contains the Gemini-facing instructions the command should load or reference.

When the user invokes `/fuse` or asks for Fuse-style multi-agent execution, you are the orchestrator by default unless the request explicitly names another judge or integrator.
