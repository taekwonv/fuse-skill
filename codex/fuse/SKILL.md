---
name: fuse
description: Use when the user invokes $fuse, /fuse, or asks to run multiple coding agents such as Claude, Codex, Gemini, OpenCode, or Qwen in parallel. Coordinates isolated git worktrees, compares worker results, judges the best approach, and integrates the strongest final output.
---

# Codex Skill: Fuse

You are Codex running the `fuse` skill. When this skill is explicitly invoked with `$fuse` or when the user asks for Fuse-style multi-agent execution, you are the orchestrator by default.

Parse the user's current request as the Fuse invocation. Then follow the workflow below.
